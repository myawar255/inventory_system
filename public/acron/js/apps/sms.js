...
class Chat {
    ...
    constructor(options = {}) {
        ...
        let sms_spinner = document.getElementById('sms_spinner');

        this.chatInput = document.querySelector('#chatInput');
        this.currentChatData = null;
        Helpers.FetchJSON($('#smsURL').val(), (data) => {
            console.log('data: ', data);

            document.getElementById('sms_spinner').style.display = 'none';
            if (data.is_error) {

                document.getElementById('messages').style.display = 'none';
                var error_page = document.getElementById('error_page');
                error_page.style.display = 'inline';
            }
            this.chatData = data.data.map((d) => {
